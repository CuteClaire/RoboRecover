#!/usr/bin/env python3
"""
Filtered OOD JSON strict repro for Unifolm-VLA: replay annotated actions -> optional wait (default 0) -> local model infer.

Outputs match openpi `eval_filtered_repro.py`: results.jsonl, summary.json, eval.log; per-row fields include
failure_detail, crash_kind, retry_count, init_*_err, infer_vs_annotated_*, video_agent paths (video on by default).
...
Uses Unifolm_VLA_Inference (no websocket).
"""
...
from experiments.LIBERO.eval_libero import get_action_state, prepare_observation, process_action
from experiments.LIBERO.libero_utils import get_libero_image, get_libero_wrist_image, quat2axisangle
...
DEFAULT_FILTERED_ROOTS = [
    str(_REPO_ROOT.parent / "openpi" / "filtered_data" / "OOD_data"),
    str(_REPO_ROOT.parent / "Being-H" / "filtered_data" / "OOD_data"),
    str(_REPO_ROOT / "filtered_data" / "OOD_data"),
]
...
def _obs_state(obs: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, float]:
    ...
    gripper = float(gripper_raw[0]) if isinstance(gripper_raw, np.ndarray) else float(gripper_raw)
...
def _frame_pair(obs: dict[str, Any]) -> tuple[np.ndarray, np.ndarray]:
    agent = np.ascontiguousarray(get_libero_image(obs))
    wrist = np.ascontiguousarray(get_libero_wrist_image(obs))
    return agent, wrist
...
def _safe_env_step(env: Any, action: list[float] | np.ndarray) -> tuple[tuple[Any, Any, bool, Any] | None, bool]:
    # Some robosuite versions throw ValueError instead of returning done=True on terminal episodes.
...
def _suite_to_rlds_dataset_name(suite: str) -> str:
    # Keep naming aligned with DATASETS/modified_libero_rlds.
    return {
        "libero_spatial": "libero_spatial_no_noops",
        "libero_object": "libero_object_no_noops",
        "libero_goal": "libero_goal_no_noops",
...
def _obs_to_rlds_state8(obs: dict[str, Any]) -> np.ndarray:
    # state = concat(eef_pos(3), axis-angle(eef_quat)(3), gripper_qpos(2)) => (8,)
    eef_pos = np.asarray(obs["robot0_eef_pos"], dtype=np.float32).reshape(-1)[:3]
    eef_quat = np.asarray(obs["robot0_eef_quat"], dtype=np.float64).reshape(4)
    eef_axis_angle = np.asarray(quat2axisangle(eef_quat), dtype=np.float32).reshape(-1)[:3]
    ...
    return np.concatenate([eef_pos, eef_axis_angle, gripper_qpos], axis=0)


def _obs_to_rlds_joint_state7(obs: dict[str, Any]) -> np.ndarray:
    ...
    *,
    ...
    return {
        "observation": {
            ...
            "state": _obs_to_rlds_state8(obs),
            "joint_state": _obs_to_rlds_joint_state7(obs),
        },
        "action": action_arr,
        "discount": 1.0,
        "reward": float(reward),
...
def _apply_unnorm_for_suite(model: Unifolm_VLA_Inference, suite: str, policy_ckpt_path: str) -> None:
    uk = SUITE_TO_UNNORM_KEY.get(suite)
    if uk is None:
        raise ValueError(f"no unnorm_key mapping for suite={suite}")
    if getattr(model, "_last_unnorm_key", None) == uk:
        return
    model.unnorm_key = uk
    model.action_norm_stats = Unifolm_VLA_Inference.get_action_stats(uk, policy_ckpt_path)
    model.state_norm_stats = Unifolm_VLA_Inference.get_state_stats(uk, policy_ckpt_path)
    model._last_unnorm_key = uk  # type: ignore[attr-defined]
...
def _classify_crash(exc: BaseException, crash_phase: str) -> str:
    ...
    if "connection" in msg or "timeout" in msg or "refused" in msg or "errno 111" in msg:
        return "infrastructure"
    if "cuda" in msg or "out of memory" in msg or "oom" in msg or "cudnn" in msg:
        return "infrastructure"
    if isinstance(exc, OSError) and getattr(exc, "errno", None) in (101, 104, 110, 111):
        return "infrastructure"
    if isinstance(exc, RuntimeError) and ("cuda" in msg or "out of memory" in msg):
        return "infrastructure"
...
def _eval_one_sample_attempt(
    ...
    # In infer_only_after_replay mode, we only save infer-phase transitions.
    ...
    _apply_unnorm_for_suite(model, suite, policy_ckpt_path)
    ...
    try:
        ...
        if not replay_done and not wait_done:
            ...
            obs_queue = deque(maxlen=window_size)
            if save_video:
                a, w = _frame_pair(obs)
            ...
            for _ in range(max_infer_steps):
                while len(obs_queue) < window_size:
                    observation, _ = prepare_observation(obs, resize_size=resize_size)
                    obs_queue.append(observation)

                if len(action_queue) == 0:
                    actions_out = get_action_state(obs_queue, task_description, model)
                    ao = np.asarray(actions_out)
                    if ao.ndim == 1:
                        action_queue.append(ao)
                    else:
                        action_queue.extend(ao)

                obs_queue.popleft()
                action = action_queue.popleft()
                action = process_action(np.asarray(action))
        ...
        # Collect RLDS episode for later TFRecord writing and optionally emit HDF5.
        if (save_rlds or save_hdf5) and rlds_steps:
            ...
            if should_save_rlds or should_save_hdf5:
                ...
                if should_save_hdf5 and hdf5_data_dir is not None and hdf5_counter is not None:
                    try:
...
def _to_summary(results: list[EvalResult]) -> dict[str, Any]:
    ...
    def _bucket(key_fn):
        ...
        for r in results:
            k = key_fn(r)
            b.setdefault(k, {})
            b[k][r.status] = b[k].get(r.status, 0) + 1
        return b
...
def _resolve_init_unnorm_key(pretrained_path: str, cli_unnorm: str | None) -> str | None:
    """
    Choose unnorm_key for Unifolm_VLA_Inference.__init__.

    When the checkpoint only has one dataset in norm_stats, None is OK (auto-pick).
    When there are multiple (Libero multi-suite), __init__ must receive a valid key;
    per-sample evaluation still switches stats via _apply_unnorm_for_suite using task_suite_name.
    """
    from unifolm_vla.model.framework.share_tools import read_mode_config

    _, norm_stats = read_mode_config(pretrained_path)
    keys = list(norm_stats.keys())
    if cli_unnorm and cli_unnorm.strip():
        uk = cli_unnorm.strip()
        if uk not in norm_stats:
            raise ValueError(
                f"--unnorm-key {uk!r} not in checkpoint norm_stats keys: {keys}"
            )
        return uk
    if len(norm_stats) == 1:
        return None
    # Multi-dataset: align default with scripts/eval_scripts/run_eval_libero.sh fallback (goal).
    default_uk = "libero_goal_no_noops"
    if default_uk not in norm_stats:
        default_uk = sorted(keys)[0]
    return default_uk
...
def main():
    parser = argparse.ArgumentParser(description="Unifolm-VLA filtered OOD repro: replay -> wait (default 0) -> infer.")
    parser.add_argument(
        "--filtered-root",
        action="append",
        default=[],
        help="Repeatable roots (default: openpi, Being-H, unifolm-vla .../filtered_data/OOD_data when present).",
    )
    parser.add_argument("--folder-prefix", action="append", default=[])
    parser.add_argument("--pretrained-path", type=str, required=True)
    parser.add_argument("--vlm-pretrained-path", type=str, default="")
    ...
    parser.add_argument("--video-root", type=str, default="eval_logs/filtered_repro_unifolm/videos")
    parser.add_argument("--output-dir", type=str, default="eval_logs/filtered_repro_unifolm")
    parser.add_argument("--run-name", type=str, default="")
    parser.add_argument("--model-tag", type=str, default="unifolm-vla")
    ...
    parser.add_argument("--hdf5-data-dir", type=str, default="")
    parser.add_argument(
        "--eval-mode",
        type=str,
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Normalization stats key for checkpoint init when norm_stats has multiple datasets "
            "(same names as eval_libero.py --args.unnorm-key). "
            "Omit to auto-pick: single-dataset ckpt uses its only key; multi-dataset defaults to "
            "libero_goal_no_noops (or first available). Per-sample suite still overrides via JSON task_suite_name."
        ),
    )
    ...
    logging.info(
        "pretrained=%s wait_after_replay=%d max_infer=%d save_video=%s wrist=%s eval_mode=%s",
    ...
    init_unnorm = _resolve_init_unnorm_key(
        args.pretrained_path,
        args.unnorm_key.strip() or None,
    )
    logging.info(
        "Loading model from %s (init unnorm_key=%s) ...",
        args.pretrained_path,
        init_unnorm,
    )
    model = Unifolm_VLA_Inference(
        policy_ckpt_path=args.pretrained_path,
        unnorm_key=init_unnorm,
        image_size=list(args.resize_size),
        vlm_pretrained_path=args.vlm_pretrained_path or None,
    )

    resize_tuple = (int(args.resize_size[0]), int(args.resize_size[1]))
