...
class PI0:
    ...
    def infer_from_robotwin(self, payload):
        instruction = payload.get("instruction")
        images = payload["images"]
        state = payload["state"]
        pi0_step = int(payload.get("pi0_step", self.pi0_step))

        if self.observation_window is None:
            if instruction is None:
                raise ValueError("instruction is required for the first infer call after reset")
            self.set_language(instruction)

        self.update_observation_window(images, state)
        return self.get_action()[:pi0_step]
