...
class Base_Task(gym.Env):
    ...
    def _set_eval_video_ffmpegs(self, ffmpegs):
        self.eval_video_ffmpegs = ffmpegs

    def _capture_eval_video_rgb(
        self,
        camera_name: str,
        rgb_cache: dict | None = None,
    ) -> np.ndarray:
        if camera_name == "observer_camera":
            return self.cameras.get_observer_rgb()

        if camera_name == "world_camera1":
            self.cameras.world_camera1.take_picture()
            rgba = self.cameras.world_camera1.get_picture("Color")
            return (rgba * 255).clip(0, 255).astype("uint8")[:, :, :3]

        if camera_name == "world_camera2":
            self.cameras.world_camera2.take_picture()
            rgba = self.cameras.world_camera2.get_picture("Color")
            return (rgba * 255).clip(0, 255).astype("uint8")[:, :, :3]

        # Replay does not call get_obs() each step, so capture fresh RGB frames.
    ...
    def _write_eval_video_frames(self):
        if getattr(self, "eval_video_ffmpegs", None):
            rgb_cache = None
            rgb_camera_names = {"head_camera", "left_camera", "right_camera"}
            if any(camera_name in rgb_camera_names for camera_name in self.eval_video_ffmpegs):
                self._update_render()
                self.cameras.update_picture()
                rgb_cache = self.cameras.get_rgb()
            for camera_name, ffmpeg in self.eval_video_ffmpegs.items():
                rgb = self._capture_eval_video_rgb(camera_name, rgb_cache=rgb_cache)
    ...
    def _del_eval_video_ffmpeg(self):
        if getattr(self, "eval_video_ffmpegs", None):
            for ffmpeg in self.eval_video_ffmpegs.values():
                ffmpeg.stdin.close()
                ffmpeg.wait()
            del self.eval_video_ffmpegs
            return
