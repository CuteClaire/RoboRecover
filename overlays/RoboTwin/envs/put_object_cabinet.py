...
class put_object_cabinet(Base_Task):
    ...
    def check_success(self):
        ...
        if not hasattr(self, "origin_z"):
            self.origin_z = object_pose[2]
        # OOD replay restores recorded actions without calling play_once(), so
        # the arm selected during the original task may not have been set.
