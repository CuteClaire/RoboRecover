...
def _skip_curobo_planner() -> bool:
    return os.environ.get("ROBOTWIN_SKIP_CUROBO_PLANNER", "").lower() in ("1", "true", "yes")
...
class Robot:
    ...
    def reset(self, scene, need_topp=False, **kwargs):
        ...
        if self.communication_flag:
            ...
            if _skip_curobo_planner():
                if self.need_topp and not hasattr(self, "left_mplib_planner"):
    ...
    def left_plan_grippers(self, now_val, target_val):
        if _skip_curobo_planner():
            return self.left_mplib_planner.plan_grippers(now_val, target_val)
        if self.communication_flag:
    ...
    def right_plan_grippers(self, now_val, target_val):
        if _skip_curobo_planner():
            return self.right_mplib_planner.plan_grippers(now_val, target_val)
        if self.communication_flag:
